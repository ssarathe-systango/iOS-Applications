...
//
//  Created by macmini01 on 08/08/22.
//

import UIKit
protocol CountryControllerDelegate: AnyObject {
...
class CountryController: UIViewController {
    ...
    //MARK: DELEGATE
    weak var delegate: CountryControllerDelegate?
...
//        self.navigationItem.title = "Select Country"
    ...
    // LIST OF COUNTRY TYPE
    var list : [Country] = [Country]()
    var filteredList: [Country] = [Country]()
}

//MARK: FUNCTION FOR COUNTRY CONFIGURATION
...
extension CountryController: UITableViewDelegate, UITableViewDataSource {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        cell?.textLabel?.text = "\(filteredList[indexPath.row].flag ?? "") \(filteredList[indexPath.row].name ?? "")"
        return cell!
    }
    
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        delegate?.sendData(country: filteredList[indexPath.row])
...
extension String {
    static func flag(for code: String) -> String? {
        ...
        guard lowercasedCode.unicodeScalars.reduce(true, {
            accum, scalar in accum &&
            isLowercaseASCIIScalar(scalar)
        }) else {
            return nil
        }
        
        let indicatorSymbols = lowercasedCode.unicodeScalars.map( {
            regionalIndicatorSymbol(for: $0)
        })
...
//        print("search = \(searchText)")
        self.filteredList = searchText.isEmpty ? list : list.filter({ ( model ) in
